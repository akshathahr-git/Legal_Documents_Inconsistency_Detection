...
def display_results(results: dict):
    ...
    with tab2:
        consistent = [r for r in results['results'] if not r['is_inconsistent']]
        if consistent:
            # Create dataframe for consistent clauses
            data = []
            for cons in consistent[:20]:  # Limit to first 20
                data.append({
                    'Document 1': cons['clause_1']['document_name'],
                    'Clause 1': cons['clause_1']['clause_number'],
                    'Document 2': cons['clause_2']['document_name'],
                    'Clause 2': cons['clause_2']['clause_number'],
                    'Similarity': f"{cons['similarity_score']:.2%}",
                    'Confidence': f"{cons['confidence']:.2%}"
                })
            
            df = pd.DataFrame(data)
            st.dataframe(df, use_container_width=True)
            
            # Show sample of consistent text
            st.markdown("### Sample Consistent Clauses")
            sample = consistent[0] if consistent else None
            if sample:
                st.markdown(f"**{sample['clause_1']['document_name']}**: {sample['clause_1']['text'][:200]}...")
                st.markdown(f"**{sample['clause_2']['document_name']}**: {sample['clause_2']['text'][:200]}...")
        else:
            st.info("No consistent clauses found above the similarity threshold")
